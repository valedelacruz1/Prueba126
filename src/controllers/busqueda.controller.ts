...
const getIncidentesPorUsuario = async (req: Request, res: Response) => {
  ...
  if (idTipoIncidente > 0 && estadoIncidenteVal > 0) {
    ...
    tipoIncidenteM = await getModeloBD("tipoIncidente",payloadTipoIncidente,"MONGO");

    estadoIncidenteM = await getModeloBD("estadoIncidente",payloadEstadoIncidente,"MONGO");
...
const getUsuariosPorRole = async (req: Request, res: Response) => {
  ...
  //Obtener la dependencia MYSQL
  let role: any = await getModeloBD(
    "role",
    payloadRole,
    "MYSQL"
  );

  let roleM: any = await getModeloBD(
    "role",
    payloadRole,
    "MONGO"
  );
