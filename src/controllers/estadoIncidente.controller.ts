...
export const getEstadoIncidente = async (req: Request, res: Response) => {
  ...
  let estadoIncidente: any = await getModeloBD('estadoIncidente',query,"MYSQL");

  //Obtener la dependencia MONGODB
  let estadoIncidenteM: any = await getModeloBD('estadoIncidente',queryMongo,"MONGO");
...
export const actualizarEstadoIncidente = async (req: Request,res: Response) => {
  ...
  try {
    ...
    await estadoIncidenteMDB.findByIdAndUpdate(estadoIncidenteMDB._id, body, {
      new: true,
    });
...
export const borrarEstadoIncidentePermanente = async (req: Request,res: Response) => {
  const { id } = req.params;
